...
namespace UnityEngine.UI.Extensions
{
    ...
    public class ScrollSnap : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollSnap
    {
        ...
        {
            //if (this.transform.parent.name == "ItemArea")

            if (!_listContainerRectTransform.rect.size.Equals(_listContainerCachedSize))// jj do odswiezenia
            {
                ...
                if (activeCount > 0)
                {
                    ...
                    if (direction == ScrollDirection.Horizontal)
                    {
                        ...
                        {
                            //if (this.transform.parent.name == "ItemArea")
                            //Debug.Log(_listContainerMaxPosition + ">>" + _itemSize);
        ...
        void LateUpdate()
        {
            UpdateListItemsSize();
            UpdateListItemPositions();
            if (_bDrag && !Input.GetMouseButton(0))
            {
                PointerEventData eventDataCurrentPosition = new PointerEventData(EventSystem.current);
                eventDataCurrentPosition.position = Input.mousePosition;
                OnEndDrag(eventDataCurrentPosition);
                    }
        ...
        public int CurrentPage()
        {
            ...
            float page = pos / _itemSize;
              ...
              //  Debug.Log(pos + "??" + _itemSize + "??" + _pages);
